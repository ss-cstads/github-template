# Deploy Fullstack — JavaScript (Express.js) + Angular + MySQL

Aplicacao web completa com 3 servicos:

```
Internet  →  Frontend (Angular)  →  Backend (Express.js)  →  MySQL
```
...
cp exemplos/fullstack-javascript/backend.nomad.hcl  .
cp exemplos/fullstack-javascript/frontend.nomad.hcl .
cp exemplos/fullstack-javascript/mysql.nomad.hcl    .
cp -r exemplos/fullstack-javascript/backend/  .
cp -r exemplos/fullstack-javascript/frontend/ .
cp exemplos/fullstack-javascript/.github/workflows/deploy.yml .github/workflows/deploy.yml
```
...
```bash
git add .
git commit -m "deploy fullstack javascript"
git push
```
...
│   └── server.js                  # Codigo-fonte Express.js
└── frontend/
