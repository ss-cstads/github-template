# Deploy Fullstack — TypeScript (Express + Prisma) + Angular + MySQL

Aplicacao web completa com 3 servicos:

```
Internet  →  Frontend (Angular)  →  Backend (Express + TypeScript + Prisma)  →  MySQL
```
...
```bash
git add .
git commit -m "deploy fullstack typescript"
git push
```
...
│   ├── prisma/
│   │   └── schema.prisma          # Modelos do banco de dados (Prisma ORM)
│   └── src/
