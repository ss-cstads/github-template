...
no cluster HashiCorp da instituição via GitHub Actions.
...
Vá em **Settings → Secrets and variables → Actions**, clique em **"New repository secret"** e adicione:
...
> **Importante:** após o primeiro push, torne sua imagem Docker pública:
...
## Usando segredos na aplicação (Vault)

Se sua aplicação precisar de senhas ou chaves secretas, elas são gerenciadas
pelo GitHub Actions — você não precisa de acesso direto ao Vault.

**1. Adicione os secrets no GitHub** (**Settings → Secrets and variables → Actions → "New repository secret"**):
...
**2. Crie uma Variable** (**Settings → Secrets and variables → Actions**, aba **"Variables" → "New repository variable"**):

| Variable | Valor |
|----------|-------|
| `SYNC_VAULT_SECRETS` | `true` |

**3. Descomente o bloco `template` no `job.nomad.hcl`** para injetar os segredos
como variáveis de ambiente na sua aplicação.

No próximo push, o GitHub Actions escreverá os secrets no Vault automaticamente
antes de fazer o deploy. Depois que os secrets estiverem criados, você pode
definir `SYNC_VAULT_SECRETS=false` para não reescrevê-los a cada deploy.
